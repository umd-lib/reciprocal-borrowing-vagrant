...
Each machine has a "borrow" service user that is created during configuration.

To access the "borrow" service account:

```
> su - borrow
Password: [Password]
```
The default password is "borrow".

The default username and password for the service user can be changed in [vagrant_env_config.sh](vagrant_env_config.sh) of the Vagrant machine.
...
The following steps configure the SP virtual machine for use as an SP client:

2) Login to the SP virtual machine and switch to the "borrow" service account:
...
```
    <ApplicationDefaults entityID="https://borrow.btaa.org/shibboleth"
                         REMOTE_USER="eppn persistent-id targeted-id">
```
